...
export const getPrizes = async (req: Request, res: Response): Promise<Response> => {
  try {
...
export const getRedemptionDates = async (req: Request, res: Response): Promise<Response> => {
  const { query } = req;
  try {
    if (!query.fan_id) {
      return res.send(400).json({error: "Redemption Date requests must include fan_id"})
    }
    const fanId = Number(query.fan_id)
    const dates = await getRedemptionDatesForFan(fanId)
    if (!dates || dates.length < 1) throw new Error('Could not get redemption dates for fan')
...
export const redeemPrizeCode = async (
  ...
  try {
    const strippedFanPartial: Partial<Fan> = {
      ...
      hometown: body.fan.hometown,
      email: body.fan.email
    }
    const validToken = await fanIdMatchesToken(strippedFanPartial)
    if (!body.prize_id) {
      return res.status(400).json({message: "Request must include a prize_id"})
    }
    if (!validToken) {
